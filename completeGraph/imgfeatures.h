...
#define FEATUREVECSIZE 36  // 128(SIFT) 36(PCA-SIFT)

#define PCAWINDOWSIZE 41
#define GRADVECSIZE 3042
#define GRADVECTHRESH 0.2
...
struct feature
{
	double x;                      /**< x coord */
	double y;                      /**< y coord */
	double xother;                 /*与x匹配的点（自己加的）*/
	double yother;                 /*与y匹配的点（自己加的）*/
...
typedef struct
{
	float x;                             // subpixel x coord
	float y;                             // subpixel y coord
	float scale;                         // sub-scale scale-value
	int octave;                          // octave index
	float orientation;                   // orientation
	float featurevector[FEATUREVECSIZE]; // feature vector
} ipoint;
...
*/
extern int import_features( char* filename, int type, struct feature** feat ,int* w, int* h);
