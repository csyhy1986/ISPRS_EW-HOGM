...
static int import_lowe_features( char*, struct feature**, int*, int*);
...
*/
int import_features( char* filename, int type, struct feature** feat, int* w, int* h )
{
	int n;
	...
	case FEATURE_OXFD:
		...
		break;
	case FEATURE_LOWE:
		n = import_lowe_features( filename, feat, w, h );
		break;
	default:
...
@return Returns the number of features imported from filename or -1 on error
*/
static int import_lowe_features( char* filename, struct feature** features, int* w, int* h )
{
	struct feature* f;
	int i, j, n, d;
	double x, y, s, o, dv;
...
static int export_lowe_features( char* filename, struct feature* feat, int n )
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		for( j = 0; j < d; j++ )
		{
			...
			if( j % 20 == 0 )
				fprintf( file, "\n" );
			fprintf( file, " %-20.12f", feat[i].descr[j] );
		}
		fprintf( file, "\n" );
	}
